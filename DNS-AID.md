...
> `naymme.vercel.app` therefore **cannot be created, period** —
...
>   "TXT fallback" below)
>
> `domainsChecked` in the scan result is `["naymme.vercel.app"]`
...
1. Attach a custom domain to the Vercel project — e.g.
   `naymme.nandodani.dev` (a name inside a zone you control).
2. In that zone, publish the two records **under the attached hostname**
   — `_index._agents.naymme.nandodani.dev` and
   `naymme._agents.naymme.nandodani.dev` — with `TargetName` set to
   the custom domain itself (or keep `naymme.vercel.app.` — both
   resolve to the service).
3. Enable DNSSEC on the zone.
4. Re-scan `https://naymme.nandodani.dev` —
...
with `TargetName = naymme.vercel.app.` is valid DNS-AID today —
resolvers that query `_index._agents.nandodani.dev` will discover the
service. It does **not** change the audit result for
`naymme.vercel.app`: the auditor never queries other domains, so
...
- **Cloudflare DNS** — full `SVCB` + `HTTPS` support and one-click
  ...
  `_index._agents.<host>` (e.g. `_index._agents.naymme`), value
  `1 <TargetName>. alpn="h2" port=443 mandatory=alpn,port well-known="api-catalog"`;
  repeat for `naymme._agents.<host>` with `bap="mcp"`. Enable DNSSEC
  under DNS → Settings.
...
`https://naymme.vercel.app` can never pass — `vercel.app` is
