  ...
  return (
    <ul aria-hidden="true">
      {Array.from({ length: count }, (_, i) => (
        <li
          key={i}
          className="flex min-h-9 items-center gap-2.5 border-t border-white/5 py-1 first:border-t-0"
        >
...
/**
 * One column per provider group (e.g. Core domains / Social media): an
 * 11px tracked header and provider rows — no card chrome; sections pack
 * into the page's CSS columns. Skeleton pulses while the async
 * RDAP/WHOIS/registry checks are in flight.
 */
